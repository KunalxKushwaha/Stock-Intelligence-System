...
async function pullSyncedDataFromServer() {
  ...
  try {
    ...
    const settings = data.settings || { risk_profile: 'balanced', theme: 'dark' };
...
// Applies a synced theme + risk profile to THIS device, used right after
// login/page load so a second device picks up what you set on the first.
...
// ---------- extend theme toggle + risk profile change with sync ----------
// Same override pattern used above for the watchlist/position functions:
// identical behavior for guests, plus a push to the server when signed in.
// Defined here (loaded after app.js) so this safely replaces the app.js
// version — nothing in app.js itself changes.
